...
class WrapperDB:
    ...
    def elencoPost(self, as_dict = False):
        ...
        try:
            ...
            sql = "SELECT Id, Autore, Testo, [Like] FROM PLAN_FB_Post ORDER BY [Like] DESC"
            cur.execute(sql)
            lista = cur.fetchall()
        except:
    ...
    def inserisciPost(self, parametri):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "INSERT INTO PLAN_FB_Post (Autore, Testo) VALUES (%s , %s)"
    ...
    def daiLikeAPost(self, id, is_like = True):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "UPDATE PLAN_FB_Post SET [Like] = "
            if is_like == True: 
                sql += "[Like] + 1 "
            else:
                sql += "[Like] - 1 "
    ...
    def eliminaPost(self, id):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "DELETE PLAN_FB_Post WHERE id = %d"
            cursore.execute(sql, id)
            c.commit()
